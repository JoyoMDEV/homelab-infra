...
# =============================================================================
#  setup-gitlab-runner-cache.sh
#  Creates the Garage bucket + a scoped access key for the GitLab Runner's
#  distributed S3 cache, and writes the credentials to Vault. The
#  ExternalSecret 'gitlab-runner-cache-secret' (namespace gitlab) picks these
#  up from there; the runner's runners.cache.secretName value points at the
#  Kubernetes Secret it renders.
#
#  IDEMPOTENT: if Vault already has 'homelab/gitlab/runner-cache-secret'
...
VAULT_NS="security"
VAULT_POD="vault-0"
VAULT_PATH="homelab/gitlab/runner-cache-secret"
GARAGE_NS="infrastructure"
GARAGE_POD="garage-0"
BUCKET="gitlab-runner-cache"
KEY_NAME="gitlab-runner-cache-key"
...
garage_exec() {
  ...
  # shell at all in this image (confirmed live in setup-supabase-storage.sh),
  # so the container's own entrypoint invokes it the same way.
...
force_sync gitlab-runner-cache-secret gitlab
