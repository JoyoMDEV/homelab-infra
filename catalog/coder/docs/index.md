...
- CloudCLI: a `coder_app` resource (`share = "owner"`) in the same template, proxied through Coder's own already-authenticated (Keycloak OIDC) session — no separate Ingress/Service/Keycloak client. The startup script backgrounds it bound to `127.0.0.1:3001` only (never the pod's external interface), guarded by a port check so it isn't started twice on a workspace restart. See `docs/coder-setup.md` section 9 for rollout/verification and section 10 for troubleshooting.
...
- **Change CloudCLI's port or restart it without a full workspace restart**: edit the `coder_app.cloudcli`/startup-script block in `k8s/coder-templates/homelab-workspace/main.tf`, push the template, `coder update homelab`. To just restart the process without a template change, kill it in the workspace terminal (`pkill -f cloudcli`) and re-run the startup script's CloudCLI block manually, or `coder restart homelab`. Its log lives at `$HOME/.cloudcli.log` on the workspace's persistent PVC.
